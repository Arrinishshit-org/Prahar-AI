...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Welfare scheme delivery &amp; beneficiary status
          </p>
        </div>
      ...
      {/* Health banner */}
      {health && (
        <div className="p-card p-4 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2.5">
            <div
              className={`p-health-dot ${health.status === 'healthy' ? 'healthy' : health.status === 'degraded' ? 'degraded' : 'critical'}`}
            />
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[
              ...
              <span
                key={label}
                className={`px-2.5 py-1 rounded-md text-xs font-semibold ${ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}
              >
                {ok ? '✓' : '✗'} {label}
              </span>
            ))}
        ...
        <div className="p-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {syncStatus.isSyncing && (
              <span className="p-badge p-badge-info flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <p className="text-base font-medium text-gray-800">
                {syncStatus.lastSync
                  ? new Date(syncStatus.lastSync).toLocaleString('en-IN')
                  : 'Never'}
              </p>
            </div>
            <div>
              ...
              <p className="text-base font-medium text-gray-800">
                {syncStatus.nextSync
                  ? new Date(syncStatus.nextSync).toLocaleString('en-IN')
                  : 'Not scheduled'}
              </p>
            </div>
          </div>
      ...
      <div className="p-card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <button className="p-btn p-btn-primary justify-start gap-2">
          ...
          <button className="p-btn p-btn-secondary justify-start gap-2">
