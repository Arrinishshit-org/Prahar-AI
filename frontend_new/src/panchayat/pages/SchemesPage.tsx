...
export default function SchemesPage() {
  ...
  const handleSync = async () => {
    if (!confirm('Fetch the latest schemes from India.gov.in? This may take a few minutes.'))
      return;
    setSyncing(true);
    try {
      await triggerSync();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-stat-card green">
          ...
          <p className="p-stat-value">{activeSchemes.length.toLocaleString('en-IN')}</p>
        </div>
            ...
            <tbody>
              {filtered.slice(0, 200).map((scheme) => (
                <tr key={scheme.scheme_id}>
                  ...
                  <td className="text-sm text-gray-600 max-w-[160px] truncate">
                    {scheme.ministry || '—'}
                  </td>
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-2.5 border-t border-gray-100">
            <p className="text-xs text-gray-400">
              Showing {Math.min(filtered.length, 200)} of {filtered.length} scheme
              {filtered.length !== 1 ? 's' : ''}
